...
export const DatasetDeleteDialog = ({
  ...
  const handleConfirm = async () => {
    try {
      ...
      toast.error(error?.response?.data?.message || "There was an error deleting the dataset item!");
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-mainTextV1">Confirm delete dataset item</DialogTitle>
        </DialogHeader>
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-red-800">
              ...
              <p>This action cannot be undone. The dataset item will be deleted permanently.</p>
            </div>
          </div>
        </div>
